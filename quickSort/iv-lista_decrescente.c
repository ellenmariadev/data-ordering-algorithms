...
void descendingListSort(int size, int pivotStrategy) {
    int* arr = malloc(size * sizeof(int));
    char* pivotStrategies[] = {"Original", "Mediana de três", "Amostragem aleatória"};

    generateDescendingNumbers(arr, size);
    ...
    printf("\n**** Lista decrescente de tamanho [%d] com estratégia de pivô [%s] **** ", size, pivotStrategies[pivotStrategy]);
    printf("\n\nComparações: %lld\n", comparisons);
    printf("Swaps: %lld\n", swaps);
    printf("Tempo: %fs\n", runtime);
    fflush(stdout);
